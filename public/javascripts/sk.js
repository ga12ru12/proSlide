...
(function(sk, $){
     ...
     * @class Ajaxify
     * @version 0.2
     * @author <a href="mailto:michal.matuska@superkoderi.cz">Michal MatuÅ¡ka</a>
     *
     * zĂ¡vislost
     *  - jquery
     *  - history.js, https://github.com/browserstate/history.js
     *  - jquery.form.js, https://github.com/malsup/form/ ( pro odesĂ¡lĂ¡nĂ­ formulĂ¡Å™Å¯ )
     *
     * todo
     *	- otestovat a pÅ™ipravit ajaxify na formulĂ¡Å™ s file inputem
     */
    ...
    _fn.init = function()
    {
        ...
        {
            this.log('historie ani ajax nejede na file://. SpusÅ¥ si server')
            return this;
        }
        ...
        if(History.enabled === false || ( this.options.pushStateOnly && History.emulated.pushState) )
        {
            ...
            this.log('historie v prohlĂ­Å¾eÄi nenĂ­ povolena')
            return this;
        }

        if( !this.options.pageTree || !this.options.pageTree.length )
        {
            this.log('nenĂ­ pÅ™edĂ¡n strom url webu')
            return this;
        }
    ...
    _fn.getPage = function(url)
    {
        ...
        if( typeof page === 'undefined' )
        {
            ...
            // Pokud jsem naÅ¡el nejbliÅ¾Å¡Ă­ strĂ¡nku
            if(page)
            {
    ...
    _fn.initState = function()
    {
        ...
        if( !page )
        {
            this.log('aktuĂ¡lnĂ­ url nenĂ­ ve stromu strĂ¡nek')
        }
    ...
    {
        // udĂ¡lost a poslĂ¡nĂ­ objektu pro pÅ™Ă­padnou modifikaci
    ...
    _fn.getUrl = function( $element )
    {
        ...
        // pokud je element form vytvoÅ™Ă­m si a pro parsovĂ¡nĂ­ vlastnostĂ­
        // if( $element.is('form') )
        // {
        // 	created = true;
...
(function(sk, $){
    ...
    /**
     *	@public
     *	@param {String} KlĂ­Ä kterĂ½ chceme cachovat
     */
    ...
    /**
     *	@public
     *	@param {String} KlĂ­Ä k hodnotÄ› v cache
     *	@return {Boolean}
     */
    ...
    _fn.check = function()
    {
        if (!this.timer)
        {
            ...
            var fn = function()
            {
                ...
                var un;
                if(prop !== '0px' && prop !== '10px' && prop !== '20px'){
                    prop = '20px';
                }
...
(function(sk, $){
    ...
    /*
     ...
     Created:
     14.02.2012 | Michal MatuÅ¡ka (michal.matuska@superkoderi.cz)
    ...
    sk.widgets.Carousel = function (element, options)
    {
        ...
        this.options = $.extend({
            ...
            item: '>*',
            // poÄet elementÅ¯ k kterĂ© se bude posouvat
            scroll: 1,
            // aktuĂ¡lnĂ­ krok
            position: 0,
            // fullscreen reÅ¾im, bude scrollovat o procenta
            fullscreen: true,
            // skok na zaÄĂ¡tek
            repeat: false,
            // nekoneÄnĂ© scrollovĂ¡nĂ­
            infinite: false,
            // Äas za kterĂ½ se provede animace kroku. 0 vypne animaci a bude jen pÅ™epĂ­nat
            animation: 500,
            // efekt animace
            easing: Linear.easeNone,
            // osa
            axis: 'x', // x, y
            // ovlĂ¡dacĂ­ prvek pro pÅ™edchozĂ­
            pagerPrev: null,
            // ovlĂ¡dacĂ­ prvek pro dalÅ¡Ă­
            pagerNext: null,
            // vygenerovat menu pro kroky
            pagerPages: null,
            // maska krokÅ¯
            pagerMask: null,
            pagerMaskString: '{$active}/{$all}', // {$active} = aktivnĂ­ poloÅ¾ka, {$all} = celkovĂ½ poÄet
            // timeout po kterĂ© se automatiky pÅ™epne na dalÅ¡Ă­ krok. 0 znamenĂ¡ Å¾e se automaticky neposunuje
            timeout: 0,
            // callback
            onChange: null,
            // offset
            offset: 0,
            // prĂ¡zdnej element pro doplnÄ›nĂ­ kroku
    ...
    _fn.setPosition = function(pos, animate, init, animCall)
    {
        ...
        if(this.position != pos || init || animCall)
        {
            ...
            if(!animCall)
            {
                // pÅ™evrĂ¡tit pozice pokud je opakovanĂ­
                if(this.infinite)
                {
    ...
    // vytvoÅ™enĂ­ nekoneÄnĂ©ho carouselu
...
(function(sk, $){
    ...
    sk.widgets.Countdown = function(element, options)
    {
        ...
        this.options = $.extend({
            // defaultnĂ­ tÅ™Ă­da widgetu
            widgetClass: 'sk-countdown',
            // html widgetu
            widgetHtml: '<span class="sk-countdown-bar"></span>',
            // selekce elemetnÅ¯ kterĂ© se v Äase mÄ›nĂ­
            barSelector: '.sk-countdown-bar',
            // kolik Äasu se mĂ¡ odpoÄĂ­tĂ¡vat
            time: 7000
        }, options);
...
(function(sk, $){
    ...
    /*
     * VytvoÅ™Ă­ kruhovĂ½ odpoÄet Äasu. FunkÄnĂ­ je v prohlĂ­Å¾eÄĂ­ch podporujĂ­cĂ­ transformace
